...
/** DEAL-DRIVEN MODE (2026-09-07). When `revenue.dealProjection` is attached (ReportsPanel
 *  injects the Customer tab's deal projection handoff — see readDealProjection in
 *  lib/cashflow/cashProjection.js — onto the revenue object before running the P&L/CF
 *  pipeline), every revenue/COGS driver below reads from that projection instead of the
 *  old flat-rate × typed-count model: Subscription Revenue = the deals' Contract Accrual,
 *  Transaction Revenue = their Success Fee Accrual, Cost of campaigns = their Campaign
 *  COGS Accrual, and the # of Campaigns / # of Meetings driver rows become read-only
 *  echoes of the deals. The legacy rate fields are untouched in saved state and still
 *  apply for a browser with no deal projection saved yet, so nothing goes blank. */
function dealValue(revenue, path, iso) {
  const dp = revenue?.dealProjection;
  if (!dp) return null;
  let node = dp;
  for (const key of path) node = node?.[key];
  return Number(node?.[iso]) || 0;
}

export function isDealDriven(revenue) {
  return !!revenue?.dealProjection;
}

export function campaignsForMonth(revenue, iso) {
  const fromDeals = dealValue(revenue, ['campaignsByMonth'], iso);
...
export function meetingsForMonth(revenue, iso) {
  const fromDeals = dealValue(revenue, ['meetingsByMonth'], iso);
  if (fromDeals != null) return fromDeals;
...
export function upfrontRevenueForMonth(revenue, iso) {
  const fromDeals = dealValue(revenue, ['accrual', 'subscriptionByMonth'], iso);
  if (fromDeals != null) return fromDeals;
...
export function meetingRevenueForMonth(revenue, iso) {
  const fromDeals = dealValue(revenue, ['accrual', 'transactionByMonth'], iso);
  if (fromDeals != null) return fromDeals;
...
/** Cost Per Campaign for one month = Campaign Cost Rate x THIS month's campaign
 *  count. Previously lagged one month (campaigns run in month N billed as COGS in
 *  month N+1) per a 2026-08-06 confirmation — REVERSED 2026-08-20 (Kayee: "use
 *  current month # of campaign no need to use previous month"). Campaign Cost Rate
 *  can still vary by month via campaignCostRateSchedule (2026-08-07). */
export function costPerCampaignForMonth(revenue, iso) {
  const fromDeals = dealValue(revenue, ['accrual', 'cogsByMonth'], iso);
  if (fromDeals != null) return fromDeals;
...
// Moved above makeSoftwareSeedItem/SEED_COST_ITEMS (2026-09-07 fix) — SEED_COST_ITEMS
// is an eagerly-evaluated module-level const that calls makeSoftwareSeedItem, which
// calls generateId() at import time. generateId's own `idCounter` used to be declared
// with `let` further down this file; since `let`/`const` bindings (unlike function
// declarations) are NOT hoisted before their own line, evaluating SEED_COST_ITEMS
// while idCounter's declaration hadn't been reached yet threw
// "ReferenceError: Cannot access 'idCounter' before initialization" on every request
// in production (this crashed the whole page — see the 2026-09-07 Vercel runtime-error
// investigation). `generateId` itself is a function declaration (hoisted fine either
// way); it's `idCounter` that actually needed to move.
let idCounter = 0;
...
// 'software' / 'other-software' carry the extra isSoftware/driverType fields (2026-08-27)
// so they show up on the new Software tab instead of the Assumptions sidebar's
// Non-Headcount Costs cards (see CostItemsCard's `!i.isSoftware` filter) — everything
// else about them (id, category, linkedRowLabel) is untouched, so any P&L row already
// linked to one of these two items stays linked exactly as before.
function makeSoftwareSeedItem(id, name, amount, category) {
  return {
    ...makeCostItem(id, name, amount, 'Monthly', category),
    isSoftware: true,
    active: true,
    driverType: 'fixed',
    // One open-ended period covering every month (fromMonth/toMonth both blank), same
    // "always on, flat amount" behavior the pre-period single-amount shape had — see
    // lib/software/softwareData.js's makePeriod/periodForMonth (2026-08-27 rewrite).
    // Built inline rather than importing makePeriod: softwareData.js already imports
    // generateId from THIS file, so importing the other direction would be circular.
    periods: [{ id: generateId('period'), fromMonth: '', toMonth: '', amount, cadence: 'Monthly' }],
    unitLabel: '',
    unitRate: 0,
    unitsPerMonth: 0,
    revenuePercent: 0,
...
export const SEED_COST_ITEMS = [
  ...
  makeSoftwareSeedItem('software', 'Software', 1000, 'CoGS'),
  makeSoftwareSeedItem('other-software', 'Other Software', 500, 'OpEx'),
