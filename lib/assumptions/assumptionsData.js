...
/** Flips one actual month's override on/off. Turning it ON for the very first time
 *  (the iso key has never existed in campaignActualOverrides before) clears that
 *  month's stored campaignsByMonth entry — almost always still sitting at its
 ...
 *  Every later toggle for that same month (off, then on again) leaves whatever the
 *  person actually typed alone. Returns the whole new `revenue` object, ready to hand
 *  straight to onRevenueChange/setAssumptionsState. */
export function toggleCampaignActualOverride(revenue, iso) {
  const overrides = revenue.campaignActualOverrides || {};
  const isFirstTime = !(iso in overrides);
