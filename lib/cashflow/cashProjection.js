...
import {
  ...
  headcountBonusCashByCostType,
} from '../payroll/payrollData';
import { DEAL_PROJECTION_STORAGE_KEY } from '../deals/dealsData';
...
export function readCustomerInflowTotals() {
  ...
  // 2026-09-07: the Customer tab's deal-based projection (lib/deals/dealsData.js) now
  // writes DEAL_PROJECTION_STORAGE_KEY — a superset of the old inflow shape (same
  // totalsByMonth / transactionByMonth / subscriptionByMonth keys, plus cogsCashByMonth,
  // accrual.* and the campaign/meeting counts for the P&L). Preferred whenever present;
  // the legacy per-customer campaigns/meetings key below is only read for a browser that
  // saved under the old model and hasn't opened the new Customer tab yet.
  const fromDeals = readDealProjection();
  if (fromDeals) return fromDeals;
  try {
...
/** Reads the deal-based Customer projection handoff (written by lib/deals/useDealsState.js).
 *  Returns null unless a version-3 payload is present, so callers can fall back. */
export function readDealProjection() {
  if (typeof window === 'undefined') return null;
  try {
    const raw = window.localStorage.getItem(DEAL_PROJECTION_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || parsed.version !== 3 || !parsed.totalsByMonth || typeof parsed.totalsByMonth !== 'object') return null;
    if (!(Number(parsed.dealCount) > 0)) return null; // no deals yet — legacy model stays in charge
    const obj = (v) => (v && typeof v === 'object' ? v : {});
    return {
      version: 3,
      totalsByMonth: obj(parsed.totalsByMonth),
      transactionByMonth: obj(parsed.transactionByMonth),
      subscriptionByMonth: obj(parsed.subscriptionByMonth),
      cogsCashByMonth: obj(parsed.cogsCashByMonth),
      accrual: {
        subscriptionByMonth: obj(parsed.accrual?.subscriptionByMonth),
        transactionByMonth: obj(parsed.accrual?.transactionByMonth),
        cogsByMonth: obj(parsed.accrual?.cogsByMonth),
      },
      campaignsByMonth: obj(parsed.campaignsByMonth),
      meetingsByMonth: obj(parsed.meetingsByMonth),
    };
  } catch {
    return null;
  }
...
export function plAccrualForMonth(account, iso, ctx) {
  ...
  if (account.synthetic === 'payrollCogs') {
    return payrollState
      ? headcountCostByCostType(payrollState.roster, payrollState.bonuses, payrollState.assumptions, 'CoGS', iso, revenue?.dealProjection)
      : 0;
  }
        ...
        : label === 'payroll taxes' || label === 'taxes'
          ...
          : label === 'benefits'
            ...
            : label === 'bonuses' || label === 'bonus'
              ...
              : null;
    if (calc) {
      // 6th arg = campaign/meeting drivers for the milestone / per-meeting bonus plans
      // (2026-09-15) — ignored by the salary/tax/benefit calcs.
      total += calc(payrollState.roster, payrollState.bonuses, payrollState.assumptions, 'OpEx', iso, revenue?.dealProjection);
    }
  }
...
export function cashOutflowForMonth(account, timing, iso, forecastSet, ctx) {
  ...
  // Cost of campaigns (2026-09-07): when the Customer tab's deal projection is driving
  // the P&L, its cash schedule is the deal-level weighted cash-out curve (month 1 = $0,
  // then the per-term % table — see lib/deals/dealsData.js), NOT the accrual month.
  // Only the default Follow-P&L mode is redirected; an explicit quarterly/annual/manual
  // override on this account in the CF sidebar still wins.
  const dealCogsCash = ctx?.revenue?.dealProjection?.cogsCashByMonth;
  if (dealCogsCash && account?.label === 'Cost of campaigns') {
    return Number(dealCogsCash[iso]) || 0;
  }

  // Payroll Bonuses (2026-09-15): each bonus plan carries its own payout timing
  // (monthly = same month as earned, per Brennan for coordinators; quarterly = lump in
  // the quarter's last month, per Brennan for Hannah). In Follow-P&L mode the Bonuses
  // row therefore pays per PLAN, not one blanket rule — an explicit quarterly/annual/
  // manual override on the account in the CF sidebar still wins above.
  const { payrollState, sbSection, revenue } = ctx || {};
  // Same per-person payout timing for the CoGS side: the synthetic "Headcount (Payroll)"
  // COGS line = salaries/taxes/benefits (cash = accrual) + bonuses on a CASH basis.
  if (payrollState && account?.synthetic === 'payrollCogs') {
    const r = payrollState.roster;
    const b = payrollState.bonuses;
    const a = payrollState.assumptions;
    const drv = revenue?.dealProjection;
    return (
      headcountCostByCostType(r, b, a, 'CoGS', iso, drv) -
      headcountBonusByCostType(r, b, a, 'CoGS', iso, drv) +
      headcountBonusCashByCostType(r, b, a, 'CoGS', iso, drv)
    );
  }
  const label = String(account?.label ?? '').trim().toLowerCase();
  if (payrollState && sbSection != null && account?.section === sbSection && (label === 'bonuses' || label === 'bonus')) {
    let cash = headcountBonusCashByCostType(payrollState.roster, payrollState.bonuses, payrollState.assumptions, 'OpEx', iso, revenue?.dealProjection);
    for (const item of ctx?.costItems || []) {
      if (item.linkedRowLabel === account.label) cash += costItemAmountForMonth(item, iso);
    }
    return cash;
  }
