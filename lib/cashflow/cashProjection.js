...
/** Reads the Customer tab's saved monthly cash-in totals — null (not zeros) when the
 *  Customer tab has never saved in this browser, so the CF row can render visibly
 *  blank instead of a silently-wrong $0. */
export function readCustomerInflowTotals() {
  ...
  try {
    ...
    if (!parsed || parsed.version !== 1 || typeof parsed.totalsByMonth !== 'object' || !parsed.totalsByMonth) {
      return null;
    }
    return parsed.totalsByMonth;
  } catch {
    return null;
  }
}
