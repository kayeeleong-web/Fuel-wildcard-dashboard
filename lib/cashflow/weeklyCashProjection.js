...
/** Normalizes an account's payment-split config into a list of { day, pct } entries
 *  whose pct values sum to 1 (2026-08-24, Kayee: "you might do multiply payments in
 *  each month for example salaries & benefits i do payroll twice a month... give the
 *  user the options to split the P&L cadence into different weeks"). `timing.
 *  weekPlacements` (the new multi-payment array, set from the Cash Flow Assumptions
 *  sidebar's payment-split editor) wins when present; otherwise falls back to the
 *  single legacy `timing.weekPlacementDay` field as one 100% payment, so every
 *  account set up before this feature — or never touched at all — keeps behaving
 *  exactly as it did (one payment, the whole month's total, in one week). A bad/
 *  empty split (percentages summing to 0, or no entries) also falls back to a single
 *  100% payment rather than silently zeroing an account out. */
export function normalizedWeekPlacements(timing) {
  if (Array.isArray(timing?.weekPlacements) && timing.weekPlacements.length > 0) {
    const raw = timing.weekPlacements.map((p) => ({
      day: p?.day == null || p.day === '' ? null : Number(p.day),
      pct: Number(p?.pct) || 0,
    }));
    const sum = raw.reduce((s, p) => s + p.pct, 0);
    if (sum > 0) return raw.map((p) => ({ day: p.day, pct: p.pct / sum }));
    return [{ day: raw[0]?.day ?? null, pct: 1 }];
  }
  return [{ day: timing?.weekPlacementDay ?? null, pct: 1 }];
}
 ...
 *  sums each of the account's payment splits (normalizedWeekPlacements above) — for a
 *  single-payment account (the default) that's just placementMonthForWeek() finding
 *  the ONE month (if any) whose cash lands in this week: the last calendar day of the
 *  month by default (2026-08-24, Kayee: "when you say cash that's not accrual
 *  accounting... you need to put them all in the last week of the month" — replacing
 *  the old even-split-across-every-week default), or the account's own `timing.
 *  weekPlacementDay` when set. For a MULTI-payment account (e.g. payroll twice a
 *  month), each split's own day independently finds its own week and only contributes
 *  its own share (pct) of that month's total — since a month's two payment days are
 *  always more than 7 days apart, they can never both land in the same week, so each
 *  week only ever picks up the ONE split (if any) whose day falls inside it. Every
 *  other week gets $0. Applies the SAME timing config Monthly CF's sidebar already
 *  sets — set it from either tab, it drives both. */
export function cashOutflowForWeek(account, timing, weekIso, weeksInMonthCount, forecastMonthSet, ctx) {
  ...
  let total = 0;
  for (const { day, pct } of normalizedWeekPlacements(timing)) {
    if (pct <= 0) continue;
    const month = placementMonthForWeek(weekIso, day);
    if (month == null) continue;
    total += cashOutflowForMonth(account, timing, month, forecastMonthSet, ctx) * pct;
  }
  return total;
}
