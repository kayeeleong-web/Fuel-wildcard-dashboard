...
// Revenue-stream account codes, per the COA tab (Account ID → Account Name):
//   40000 Subscription Revenue · 41000 Services Revenue · 42000 Transaction Revenue
// (2026-08-20, Kayee: "give me another toggle to have this table to separate
// transactional and subscription revenue" — matches the same two streams the
// P&L/CF Assumptions already model as "Subscription Revenue" and "Transaction
// Revenue"). Exact-match on the account code, not a prefix, since 41000
// (Services) is a real third revenue account that belongs to neither stream.
 ...
 *  - keep only customer revenue accounts (account starts with "4"), optionally
 *    narrowed further to one revenue stream via `stream`
 ...
 * @param {'all'|'subscription'|'transaction'} [stream='all'] filter to one revenue stream (see REVENUE_STREAM_ACCOUNTS)
 * @returns {{ months: string[], customers: Array<{ name: string, startMonth: string, total: number, byMonth: Record<string, number> }> }}
 */
export function buildCustomerWaterfall(transactions, stream = 'all') {
  const byCustomer = new Map();
  const monthSet = new Set();
  const streamAccounts = REVENUE_STREAM_ACCOUNTS[stream];

  for (const t of transactions || []) {
    const account = String(t.account ?? '').trim();
    if (!account.startsWith('4')) continue;
    if (streamAccounts && !streamAccounts.includes(account)) continue;
