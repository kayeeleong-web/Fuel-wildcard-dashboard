...
/** Same person across several salary lines = same normalised full name (however the
 *  lines were added); falls back to personId / id. Shared by the Employees card's
 *  blocks and the Bonus card's one-row-per-person list. */
export function personKeyOf(r) {
  const n = String(r?.name || '').trim().toLowerCase().replace(/\s+/g, ' ');
  if (n) return `name:${n}`;
  return r?.personId || r?.id;
}

/** Person-level activity: true if ANY of this person's roster lines is active in the
 *  month. A bonus is attached to one line, but the person is on payroll across all their
 *  lines, so a raise (new line) must not switch their bonus off. */
export function personActiveInMonth(employee, roster, iso) {
  if (!employee) return false;
  if (!roster || employee.isRamp) return isActiveInMonth(employee, iso);
  const key = personKeyOf(employee);
  return roster.some((r) => personKeyOf(r) === key && isActiveInMonth(r, iso));
}

/** Default bonus type from a title — anyone with "coordinator" in the title goes on the
 *  Campaign milestone + meetings plan (2026-09-17, Kayee: "if title has coordinator then
 *  put them under campaign milestone plus meeting automatically, but if we want to
 *  override we could"). Returns null when no rule matches. */
export function defaultBonusTypeForTitle(title) {
  const t = String(title || '').toLowerCase();
  if (/coordinator/.test(t) && !/creative/.test(t)) return 'coordinator';
  return null;
}
...
/** ONE bonus row PER PERSON (2026-09-15 v4 — Kayee: "keep it simple: each person's
 *  name just like the employee section, and a dropdown to select how much they get per
 *  campaign or per meeting"). A bonus = { id, employeeId, type, ...amounts, payout }.
 *
 *  Types (from the Sept 10 Wildcard sync + Hannah Merrill's Slack reply):
 *   - coordinator  : campaign milestone + meetings. "$X every N campaigns sent" (Hannah:
 *                    $1k / 55, the same for every junior coordinator) PLUS "$Y per meeting
 *                    from their own campaigns" ($100). Each coordinator's campaigns and
 *                    meetings = the company's projected campaigns/meetings that month
 *                    (Customer tab deals) ÷ the number of active people on a campaign-type
 *                    plan. Milestone part × the global Milestone Hit Rate (90%).
 *   - teamMilestone: senior coordinators — "$X every N TEAM campaigns" (no split) × Hit
 *                    Rate, plus the same $Y per meeting.
 *   - fixed        : annual $ × Bonus Attainment % ÷ 12 (the original model).
 *   - quarterly    : $ per quarter (Brennan: Hannah ≈ $10k/qtr). Accrues ÷3 monthly; cash
 *                    in the quarter's last month when payout = 'quarterly'.
 *  Fields: amount (fixed $/yr · quarterly $/qtr · $ per milestone for campaign types),
 *  per (campaigns per milestone), perMeeting ($ per meeting), payout ('monthly' |
 *  'quarterly'). Only the fields the type uses are shown/used. */
export const BONUS_TYPES = [
  { id: 'none', label: 'No bonus' },
  { id: 'coordinator', label: 'Campaign milestone + meetings' },
  { id: 'teamMilestone', label: 'Team milestone + meetings' },
  { id: 'fixed', label: 'Fixed annual' },
  { id: 'quarterly', label: 'Fixed quarterly' },
];

export function bonusTypeOf(bonus) {
  return BONUS_TYPES.some((t) => t.id === bonus?.type) ? bonus.type : 'none';
}

export const CAMPAIGN_BONUS_TYPES = ['coordinator', 'teamMilestone'];
...
/** Campaign/meeting drivers. Callers that already have the Customer tab's deal
 *  projection (cashProjection via ctx.revenue.dealProjection) pass it in; otherwise
 *  this reads the same localStorage handoff the P&L uses, memoised on the raw string.
 *  Empty maps when no deals yet — a campaign plan then contributes $0, visibly. */
...
/** Heads sharing the company's campaigns/meetings in a month = every active person on
 *  a campaign-type plan (a ramp role counts its running headcount). This is what each
 *  coordinator's individual campaign count is the company total divided by. */
export function campaignBonusHeadcount(bonuses, roster, iso) {
  let n = 0;
  for (const b of bonuses || []) {
    if (!CAMPAIGN_BONUS_TYPES.includes(bonusTypeOf(b))) continue;
    const emp = roster.find((r) => r.id === b.employeeId);
    if (!emp || !personActiveInMonth(emp, roster, iso) || !bonusWindowAllows(b, iso)) continue;
    n += emp.isRamp ? cumulativeHeadcountFor(emp, iso) : 1;
  }
  return n;
}

/** Campaigns one person is measured on in a month (per head). */
export function campaignsPerPersonFor(bonuses, roster, iso, drivers) {
  const d = resolveBonusDrivers(drivers);
  const heads = campaignBonusHeadcount(bonuses, roster, iso);
  return heads > 0 ? (Number(d.campaignsByMonth[iso]) || 0) / heads : 0;
}
export function meetingsPerPersonFor(bonuses, roster, iso, drivers) {
  const d = resolveBonusDrivers(drivers);
  const heads = campaignBonusHeadcount(bonuses, roster, iso);
  return heads > 0 ? (Number(d.meetingsByMonth[iso]) || 0) / heads : 0;
}
...
/** 'headcount' (default) | 'campaigns'. Headcount = Hannah's rule of thumb: ~90% of the
 *  coordinators hit their milestone in a month, so the pool is round(heads × hit rate)
 *  WHOLE people × $ per milestone, no campaign data needed. Campaigns = the bottom-up
 *  split of the Customer tab's projected campaigns (needs deal data + attribution). */
export function milestoneModeOf(assumptions) {
  return assumptions?.milestoneMode === 'campaigns' ? 'campaigns' : 'headcount';
}

/** Whole-person count of coordinators expected to hit their milestone in a month
 *  (2026-09-17, Kayee: "only full person — 9 people is not 7.something"). Standard
 *  rounding so a single coordinator at 90% still counts as 1. */
export function milestoneAchieversFor(bonuses, roster, iso, assumptions) {
  const heads = coordinatorHeadcount(bonuses, roster, iso);
  return heads > 0 ? Math.round(heads * hitRateOf(assumptions)) : 0;
}

/** Active heads on the per-person coordinator plan only (teamMilestone excluded). */
export function coordinatorHeadcount(bonuses, roster, iso) {
  let n = 0;
  for (const b of bonuses || []) {
    if (bonusTypeOf(b) !== 'coordinator') continue;
    const emp = roster.find((r) => r.id === b.employeeId);
    if (!emp || !personActiveInMonth(emp, roster, iso) || !bonusWindowAllows(b, iso)) continue;
    n += emp.isRamp ? cumulativeHeadcountFor(emp, iso) : 1;
  }
...
/** One person's bonus ACCRUAL for one month (what the P&L books). `bonuses` (all rows)
 *  is needed for the campaign split; `roster` for the active-heads count. */
/** A bonus row's own optional date window — defaults to the person's roster dates; an
 *  override here narrows it (e.g. Hannah's quarterly plan ending when her role changes). */
export function bonusWindowAllows(bonus, iso) {
...
export function bonusMonthlyFlow(bonus, employee, iso, assumptions, drivers, roster, bonuses) {
  if (!bonus || !employee || bonus.employeeId !== employee.id || !personActiveInMonth(employee, roster, iso)) return 0;
  ...
  const d = resolveBonusDrivers(drivers);
  const perMeeting = Number(bonus.perMeeting) || 0;
  const per = Number(bonus.per) || 0;
  const all = bonuses || [bonus];
  const r = roster || [employee];
  const meetingPart = meetingsPerPersonFor(all, r, iso, drivers) * perMeeting * heads;
  if (type === 'teamMilestone') {
    const teamCampaigns = Number(d.campaignsByMonth[iso]) || 0;
    const milestonePart = per > 0 ? (teamCampaigns / per) * amount * hitRateOf(assumptions) * heads : 0;
    return milestonePart + meetingPart;
  }
  // coordinator
  if (milestoneModeOf(assumptions) === 'headcount') {
    // Pool = round(coordinators × hit rate) whole people × $ per milestone (one milestone
    // a month); each coordinator carries an equal share so the SECTION total is the
    // whole-person number and per-person rows are that person's expected value.
    const total = coordinatorHeadcount(all, r, iso);
    const achievers = milestoneAchieversFor(all, r, iso, assumptions);
    const milestonePart = total > 0 ? (achievers * amount * heads) / total : 0;
    return milestonePart + meetingPart;
  }
  const myCampaigns = campaignsPerPersonFor(all, r, iso, drivers) * heads;
  const milestonePart = per > 0 ? (myCampaigns / per) * amount * hitRateOf(assumptions) : 0;
  return milestonePart + meetingPart;
}

/** One person's bonus CASH for one month. payout 'quarterly' = the quarter's accrual
 *  lands in Mar/Jun/Sep/Dec, $0 in the other two. */
export function bonusCashFlow(bonus, employee, iso, assumptions, drivers, roster, bonuses) {
  const payout = bonus?.payout || (bonusTypeOf(bonus) === 'quarterly' ? 'quarterly' : 'monthly');
  if (payout !== 'quarterly') return bonusMonthlyFlow(bonus, employee, iso, assumptions, drivers, roster, bonuses);
  const [y, m] = iso.split('-').map(Number);
  if (m % 3 !== 0) return 0;
  let sum = 0;
  for (let mm = m - 2; mm <= m; mm++) {
    sum += bonusMonthlyFlow(bonus, employee, `${y}-${String(mm).padStart(2, '0')}`, assumptions, drivers, roster, bonuses);
  }
  return sum;
}

/** Sum of a person's bonus rows for the month (Total Comp / Summary cards). */
export function employeeBonusMonthly(bonuses, employee, iso, assumptions, drivers, roster) {
  let total = 0;
  for (const b of bonuses || []) {
    if (b && b.employeeId === employee?.id) total += bonusMonthlyFlow(b, employee, iso, assumptions, drivers, roster, bonuses);
  }
  return total;
}
...
/** Short human terms for a bonus row. */
export function describeBonus(bonus, assumptions) {
  const type = bonusTypeOf(bonus);
  if (type === 'none') return '—';
  if (type === 'fixed') return `${money(bonus.amount)} / yr`;
  if (type === 'quarterly') return `${money(bonus.amount)} / quarter`;
  const mtg = Number(bonus.perMeeting) ? ` + ${money(bonus.perMeeting)} / meeting` : '';
  if (type === 'teamMilestone') return `${money(bonus.amount)} per ${bonus.per || '?'} team campaigns${mtg}`;
  if (milestoneModeOf(assumptions) === 'headcount') return `${money(bonus.amount)} / milestone (monthly)${mtg}`;
  return `${money(bonus.amount)} per ${bonus.per || '?'} campaigns${mtg}`;
}

/** Plain-English formula for a row's tooltip. */
export function explainBonus(bonus, assumptions) {
  ...
  switch (type) {
    case 'coordinator':
      if (milestoneModeOf(assumptions) === 'headcount')
        return `round(active coordinators × ${hit}%) whole people × ${money(bonus.amount)} per milestone (≈ monthly), shared equally per coordinator, + meetings share × ${money(bonus.perMeeting)}.`;
      return `(company projected campaigns ÷ people on campaign plans) ÷ ${bonus.per || '?'} × ${money(bonus.amount)} × ${hit}% hit rate, + (company projected meetings ÷ people on campaign plans) × ${money(bonus.perMeeting)}.`;
    case 'teamMilestone':
      return `company projected campaigns ÷ ${bonus.per || '?'} × ${money(bonus.amount)} × ${hit}% hit rate (team goal, no split), + meetings share × ${money(bonus.perMeeting)}.`;
    case 'quarterly':
      return `${money(bonus.amount)} per quarter — accrues ÷3 monthly; cash in Mar/Jun/Sep/Dec when Paid = Quarterly.`;
    case 'fixed':
      return `${money(bonus.amount)} × Bonus Attainment ÷ 12 each month.`;
    default:
      return 'No bonus.';
  }
}
...
/**
 * Default CoGS/OpEx classification + department from a job title — the list Gohar
 * Sahakyan confirmed on Slack (2026-09-16) off Brennan's Sept 10 breakdown. Used only
 * as a DEFAULT when a title is typed on a roster row (2026-09-17, Kayee: "default
 * coordinator and the others to CoGS... just to avoid error, but if they want to
 * override they could") — the CoGS/OpEx and % fields stay fully editable, and once a
 * row's classification has been edited by hand (`costTypeManual`) title changes leave it
 * alone. Matching is by keyword, case-insensitive, first rule wins; returns null when a
 * title matches nothing so the caller leaves the row as is.
 */
const TITLE_CLASSIFICATION_RULES = [
  // OpEx roles first so "Operations Assistant" doesn't get caught by a production rule
  { match: ['co-founder', 'cofounder', 'founder', 'ceo', 'coo', 'cfo'], costType: 'OpEx', cogsPercent: 0, department: 'G&A' },
  { match: ['sales', 'account executive', 'bdr', 'sdr'], costType: 'OpEx', cogsPercent: 0, department: 'S&M' },
  { match: ['engineer', 'developer', 'software'], costType: 'OpEx', cogsPercent: 0, department: 'Engineering' },
  { match: ['intern', 'operations assistant', 'ops assistant', 'strategy & operations'], costType: 'OpEx', cogsPercent: 0, department: 'Production' },
  // Split roles
  { match: ['designer'], costType: 'CoGS', cogsPercent: 75, department: 'Creative' },
  // CoGS roles — the people who produce/fulfil campaigns
  { match: ['creative', 'strategist'], costType: 'CoGS', cogsPercent: 100, department: 'Creative' },
  { match: ['coordinator', 'last mile', 'logistics', 'production'], costType: 'CoGS', cogsPercent: 100, department: 'Production' },
];
export function classificationForTitle(title) {
  const t = String(title || '').toLowerCase().trim();
  if (!t) return null;
  for (const rule of TITLE_CLASSIFICATION_RULES) {
    // Whole-word match so "coordinator" doesn't trip the "coo" rule.
    if (rule.match.some((kw) => new RegExp(`(^|[^a-z])${kw.replace(/[-&]/g, '\\$&')}([^a-z]|$)`).test(t))) {
      return { costType: rule.costType, cogsPercent: rule.cogsPercent, department: rule.department };
    }
  }
  return null;
}
...
export function baseSalaryMonthlyFor(employee, iso) {
  if (employee.isRamp) {
    const count = cumulativeHeadcountFor(employee, iso);
...
export function headcountBonusByCostType(roster, bonuses, assumptions, costType, iso, drivers) {
  ...
  for (const bonus of bonuses || []) {
    ...
    const share = costTypeShare(emp, costType);
    if (share <= 0) continue;
    total += bonusMonthlyFlow(bonus, emp, iso, assumptions, drivers, roster, bonuses) * share;
  }
  return total;
}
...
export function headcountBonusCashByCostType(roster, bonuses, assumptions, costType, iso, drivers) {
  ...
  for (const bonus of bonuses || []) {
    ...
    const share = costTypeShare(emp, costType);
    if (share <= 0) continue;
    total += bonusCashFlow(bonus, emp, iso, assumptions, drivers, roster, bonuses) * share;
  }
  return total;
}
...
function makeBonus(employeeId, type, fields = {}) {
  return { id: `bonus-${employeeId}`, employeeId, type, amount: 0, per: 0, perMeeting: 0, payout: type === 'quarterly' ? 'quarterly' : 'monthly', ...fields };
}
const COORDINATOR = { amount: 1000, per: 55, perMeeting: 100 }; // Hannah, Sept 2026 Slack

export const SEED_BONUSES = [
  // Hannah (Brennan, Sept 10 sync): ≈$10k per quarter, paid quarterly. Plan expected to
  // change in ~2-3 months — edit the row then.
  makeBonus('hannah-merrill', 'quarterly', { amount: 10000 }),
  // Campaign coordinators (Hannah): $1k every 55 campaigns sent + $100 per meeting.
  makeBonus('vaughn', 'coordinator', COORDINATOR),
  makeBonus('cristina', 'coordinator', COORDINATOR),
  makeBonus('britton', 'coordinator', COORDINATOR),
  makeBonus('mia', 'coordinator', COORDINATOR),
  makeBonus('cc-hires', 'coordinator', COORDINATOR),
  // Creative strategists — Shane's team, structure PENDING; original fixed figure kept.
  makeBonus('dylan-smith', 'fixed', { amount: 12000 }),
  makeBonus('ethan', 'fixed', { amount: 12000 }),
  makeBonus('junior-creative-hire', 'fixed', { amount: 12000 }),
  // Everyone else: original fixed-annual figures.
  makeBonus('naama', 'fixed', { amount: 150000 }),
  makeBonus('sales-hires', 'fixed', { amount: 150000 }),
  makeBonus('cs-hires', 'fixed', { amount: 50000 }),
  makeBonus('el', 'fixed', { amount: 50000 }),
  makeBonus('head-of-ops', 'fixed', { amount: 50000 }),
  makeBonus('neel', 'fixed', { amount: 30000 }),
];
...
export function migratePayrollState(loaded) {
  ...
  // Bonus → one row per person (2026-09-15 v4). Handles every earlier shape:
  //  a) original `{employeeId, bonusAmount}` rows (no `type`)
  //  b) the same-day per-person typed rows (`type` + amountPerMilestone / amountPerQuarter…)
  //  c) the same-day role-group rows (`groupLabel` + `memberIds`)
  // Current rows carry `employeeId` + a BONUS_TYPES id + `amount`.
  if (Array.isArray(next.bonuses) && next.bonuses.length) {
    const roster = Array.isArray(next.roster) ? next.roster : [];
    const has = (id) => roster.some((r) => r && r.id === id);
    const isCurrent = (b) => b && b.employeeId && BONUS_TYPES.some((t) => t.id === b.type) && 'amount' in b && !Array.isArray(b.memberIds);
    if (!next.bonuses.every(isCurrent)) {
      const out = new Map(); // employeeId -> row
      const put = (row) => { if (has(row.employeeId) && !out.has(row.employeeId)) out.set(row.employeeId, row); };
      // Seeds first for the people the conversations covered (Hannah, coordinators).
      for (const sb of SEED_BONUSES) if (['quarterly', 'coordinator'].includes(sb.type)) put({ ...sb });
      for (const b of next.bonuses) {
        if (!b) continue;
        if (Array.isArray(b.memberIds)) {
          // (c) group row → one row per member
          for (const id of b.memberIds) {
            const t = b.type === 'milestone' || b.type === 'perMeeting' ? 'coordinator' : b.type === 'teamMilestone' ? 'teamMilestone' : b.type === 'quarterly' ? 'quarterly' : 'fixed';
            const existing = out.get(id);
            if (existing && CAMPAIGN_BONUS_TYPES.includes(existing.type) && b.type === 'perMeeting') { existing.perMeeting = Number(b.amount) || 100; continue; }
            if (existing) continue;
            put(makeBonus(id, t, t === 'coordinator' ? { amount: Number(b.amount) || 1000, per: Number(b.per) || 55, perMeeting: 100 } : { amount: Number(b.amount) || 0, per: Number(b.per) || 0 }));
          }
        } else if (b.employeeId) {
          // (a)/(b) per-person legacy rows
          const amt = Number(b.amount ?? b.bonusAmount ?? b.amountPerQuarter ?? b.amountPerMilestone) || 0;
          if (b.type === 'quarterly') put(makeBonus(b.employeeId, 'quarterly', { amount: amt }));
          else if (b.type === 'milestone' || b.type === 'perMeeting') put(makeBonus(b.employeeId, 'coordinator', COORDINATOR));
          else if (amt > 0) put(makeBonus(b.employeeId, 'fixed', { amount: amt }));
        }
      }
      next = { ...next, bonuses: [...out.values()] };
    }
  }
