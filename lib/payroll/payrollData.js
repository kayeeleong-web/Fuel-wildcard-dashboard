...
function baseSalaryMonthlyFor(employee, iso) {
  ...
  if (!isActiveInMonth(employee, iso)) return 0;
  return (Number(employee.baseSalary) || 0) / 12;
}

/** Headcount cost split into its 3 components, one costType at a time (2026-08-10,
 *  Kayee: "get the salaries, payroll taxes and benefit separated... in cogs payroll no
 *  need to have division for benefit and payroll tax, only in opex you need"). These
 *  three always sum back to exactly headcountCostByCostType's own total for the same
 *  costType/month — same roster/bonus loop, just split by which part of the formula
 *  produced each dollar instead of returning one lump sum:
 *   - Salaries = base ÷ 12 (or the whole manual override, if that cell has one) + every
 *     linked Bonus $ flow. Bonus is folded in here rather than given its own line
 *     (Kayee's own call, 2026-08-10) — it carries no tax/benefit load of its own, so
 *     there was no natural 4th bucket for it to sit in.
...
export function headcountSalariesByCostType(roster, bonuses, assumptions, costType, iso) {
  ...
  for (const emp of roster) {
    if (emp.costType !== costType) continue;
    const override = emp.monthlyOverrides && emp.monthlyOverrides[iso];
    total += override != null ? override : baseSalaryMonthlyFor(emp, iso);
  }
  for (const bonus of bonuses) {
    const emp = roster.find((r) => r.id === bonus.employeeId);
    if (!emp || emp.costType !== costType) continue;
...
export function headcountPayrollTaxesByCostType(roster, bonuses, assumptions, costType, iso) {
  ...
  for (const emp of roster) {
    if (emp.costType !== costType) continue;
    const override = emp.monthlyOverrides && emp.monthlyOverrides[iso];
    if (override != null) continue; // whole override already counted under Salaries
    total += baseSalaryMonthlyFor(emp, iso) * ((assumptions.taxRate || 0) / 100);
  }
  return total;
}
...
export function headcountBenefitsByCostType(roster, bonuses, assumptions, costType, iso) {
  ...
  for (const emp of roster) {
    ...
    const override = emp.monthlyOverrides && emp.monthlyOverrides[iso];
    if (override != null) continue; // whole override already counted under Salaries
    total += baseSalaryMonthlyFor(emp, iso) * ((assumptions.benefits || 0) / 100);
  }
  return total;
}
