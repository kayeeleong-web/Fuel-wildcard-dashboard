...
export const SEED_BONUSES = [
  ...
  // Campaign coordinators (Hannah, Slack): "every 55 campaigns sent they earn $1k"
  // (individual milestone) + "$100 per meeting" from their own campaigns. Seniors, when
  // tagged, get their own plan (Team milestone type) via "+ New plan" — Hannah's team-
  // output threshold isn't confirmed yet, so it isn't seeded.
  makePlan('plan-jr-coord-ms', 'Campaign Coordinators', JUNIOR_COORDINATORS, 'milestone', 1000, { per: 55 }),
  makePlan('plan-jr-coord-mtg', 'Campaign Coordinators', JUNIOR_COORDINATORS, 'perMeeting', 100),
...
export function migratePayrollState(loaded) {
  ...
  if (Array.isArray(next.bonuses) && next.bonuses.length && !next.bonuses.some((b) => b && Array.isArray(b.memberIds))) {
    ...
    // Seed groups whose people aren't in this roster are dropped, so the card isn't
    // cluttered with empty groups.
    const plans = SEED_BONUSES.map((p) => ({ ...p, memberIds: p.memberIds.filter(has) })).filter((p) => p.memberIds.length > 0);
