...
import { migratePayrollState, migrateRampRoles, seedPayrollState } from './payrollData';
...
export function usePayrollState() {
  const planning = usePlanningState(STORAGE_KEY, {
    ...
    // Ramp-role retrofit first, then the 2026-09-15 bonus-plan / classification migration.
    migrate: (loaded) => migratePayrollState(loaded?.roster ? { ...loaded, roster: migrateRampRoles(loaded.roster) } : loaded),
  });
