...
export function usePayrollState() {
  ...
  // Visible save confirmation (2026-08-24, Kayee: "it should also show up in each tab
  // that needs to have entries so that everything can be saved") — same pattern as
  // useAssumptionsState's lastSavedAt/saveNow, so the Payroll sub-tab gets the same
  // "Save" + "Saved <time>" confirmation as P&L.
  ...
  useEffect(() => {
    ...
    try {
      ...
      // lastSavedAt deliberately NOT updated here, matching useAssumptionsState.
    }
  }, [state, hydrated]);
  ...
  // Manual re-flush for the Save button — see useAssumptionsState.js for why this
  // exists even though every edit already auto-saves.
  ...
  return { state, setState, hydrated, resetToSeed, lastSavedAt, saveNow };
}
