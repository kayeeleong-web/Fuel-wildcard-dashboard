 ...
 *
 * Every driver type is period-dated now. Usage/perSeat were deliberately left as one flat
 * rate each at first, but that was reversed on 2026-09-07 (Kayee: "you didn't do that for
 * usage and per seat... per seat could be $200 from January to February, could be $250
 * for March to April") — `usagePeriods` (unitRate + unitsPerMonth per period) and
 * `seatPeriods` (seatRate per period) follow the exact same From/To shape as Fixed's
 * `periods`; the old flat unitRate/unitsPerMonth/seatRate fields survive only as the
 * fallback a pre-rewrite item is seeded from. percentRevenue got the same period treatment
 * as Fixed on 2026-08-31 (Kayee: "for percentage of revenue I also want the ability to
 * select period") — a vendor whose take-rate steps up or down partway through the year
 * is a second `percentPeriods` entry, same "From/To + one number, see every period at
 * once" shape as Fixed's `periods`, just carrying `revenuePercent` instead of
 * `amount`/`cadence` (there's no cadence to spread — a % of that month's revenue is
 * already a monthly figure, nothing to divide).
 */
...
/** One % of Revenue period: a take-rate effective from `fromMonth` through `toMonth`
 *  inclusive, or ongoing if `toMonth` is blank — same shape and "newest fromMonth wins
 *  on overlap" rule as Fixed's makePeriod/periodForMonth above, minus `amount`/`cadence`
 *  (nothing to spread; the % is already applied fresh to whatever that month's revenue
 *  turns out to be). */
export function makePercentPeriod(overrides = {}) {
  return { id: generateId('period'), fromMonth: '', toMonth: '', revenuePercent: 0, ...overrides };
}

/** One Usage period: $/unit × units per month, effective from `fromMonth` through
 *  `toMonth` (2026-09-07, Kayee: "you didn't do that for usage and per seat... per seat
 *  could be $200 from January to February, could be $250 for March to April"). Same
 *  shape/overlap rule as the Fixed and % of Revenue periods. */
export function makeUsagePeriod(overrides = {}) {
  return { id: generateId('period'), fromMonth: '', toMonth: '', unitRate: 0, unitsPerMonth: 0, ...overrides };
}

/** One Per-Seat period: $/seat/month effective from `fromMonth` through `toMonth`. The
 *  seat COUNT still comes live from the Payroll roster for the chosen department — only
 *  the rate is period-dated. */
export function makeSeatPeriod(overrides = {}) {
  return { id: generateId('period'), fromMonth: '', toMonth: '', seatRate: 0, ...overrides };
}
...
function usageAmountForMonth(item, iso) {
  ...
  // Period-dated since 2026-09-07 (usagePeriods); falls back to the flat unitRate ×
  // unitsPerMonth pair for an item saved before that and not yet self-healed.
  const period = item.usagePeriods && item.usagePeriods.length ? periodForMonth(item.usagePeriods, iso) : null;
  if (item.usagePeriods && item.usagePeriods.length && !period) return 0;
  const units = Number(period ? period.unitsPerMonth : item.unitsPerMonth) || 0;
  const rate = Number(period ? period.unitRate : item.unitRate) || 0;
  return units * rate;
}

function percentRevenueAmountForMonth(item, iso, revenue) {
  if (!isSoftwareItemDue(item, iso)) return 0;
  // periodForMonth only reads fromMonth/toMonth, so it works unchanged for
  // percentPeriods too. Falls back to the old flat `revenuePercent` field for any item
  // saved before this period rewrite that hasn't been opened (and so self-healed) yet.
  const period = item.percentPeriods && item.percentPeriods.length ? periodForMonth(item.percentPeriods, iso) : null;
  const pct = period ? Number(period.revenuePercent) || 0 : Number(item.revenuePercent) || 0;
...
function perSeatAmountForMonth(item, iso, roster) {
  if (!isSoftwareItemDue(item, iso)) return 0;
  // Period-dated rate since 2026-09-07 (seatPeriods); flat seatRate is the pre-rewrite
  // fallback. A period list with no period covering this month = $0 for the month.
  const period = item.seatPeriods && item.seatPeriods.length ? periodForMonth(item.seatPeriods, iso) : null;
  if (item.seatPeriods && item.seatPeriods.length && !period) return 0;
  const rate = Number(period ? period.seatRate : item.seatRate) || 0;
...
export function recomputeSoftwareSchedules(costItems, ctx = {}) {
  return costItems.map((item) => {
    ...
    // Self-heal a Fixed-driver item with no periods (2026-08-31 — Kayee: "I click edit
    // but there's no where to put the amount"). Root cause: any vendor whose `periods`
    // array is empty — e.g. saved before the multi-period rewrite, or left blank after
    // removing its only period — renders the Edit panel with nothing but an "+ Add
    // Period" button, no amount box in sight. Every Fixed item gets at least one open
    // period here so the box is always there the moment the panel opens.
    const periods =
      item.driverType === 'fixed' && (!item.periods || !item.periods.length)
        ? [makePeriod({ fromMonth: currentIsoMonth(), amount: 0 })]
        : item.periods;
    // Same self-heal for % of Revenue's periods (2026-08-31 rewrite) — an item saved
    // under the old flat `revenuePercent` field, or one whose only period got removed,
    // gets seeded from that flat value so nothing silently drops to 0% on the switch.
    const percentPeriods =
      item.driverType === 'percentRevenue' && (!item.percentPeriods || !item.percentPeriods.length)
        ? [makePercentPeriod({ fromMonth: currentIsoMonth(), revenuePercent: Number(item.revenuePercent) || 0 })]
        : item.percentPeriods;
    // Same self-heal for Usage and Per Seat (2026-09-07 period rewrite) — seeded from the
    // old flat fields so an existing vendor keeps its exact number on the switch.
    const usagePeriods =
      item.driverType === 'usage' && (!item.usagePeriods || !item.usagePeriods.length)
        ? [makeUsagePeriod({ fromMonth: item.unitRate || item.unitsPerMonth ? '' : currentIsoMonth(), unitRate: Number(item.unitRate) || 0, unitsPerMonth: Number(item.unitsPerMonth) || 0 })]
        : item.usagePeriods;
    const seatPeriods =
      item.driverType === 'perSeat' && (!item.seatPeriods || !item.seatPeriods.length)
        ? [makeSeatPeriod({ fromMonth: item.seatRate ? '' : currentIsoMonth(), seatRate: Number(item.seatRate) || 0 })]
        : item.seatPeriods;
    const withPeriods = { ...item, periods, percentPeriods, usagePeriods, seatPeriods };
    const amountSchedule = {};
    for (const iso of MONTHS) {
      amountSchedule[iso] = softwareAmountForMonth(withPeriods, iso, ctx);
    }
...
function makeSoftwareItem(overrides) {
  return {
    ...
    // Usage: label is per vendor; rate × units are period-dated (usagePeriods, lazily
    // seeded by recomputeSoftwareSchedules' self-heal). unitRate/unitsPerMonth stay only
    // as the pre-rewrite fallback fields.
    unitLabel: '',
    unitRate: 0,
    unitsPerMonth: 0,
    usagePeriods: [],
    // % of revenue: lazily seeded (see recomputeSoftwareSchedules' self-heal) rather
    // than pre-filled here, same "first Edit click fills it in" pattern as Fixed's
    // periods — revenuePercent stays only as the pre-rewrite fallback field.
    percentPeriods: [],
    revenuePercent: 0,
    // Per seat: department is per vendor; the $/seat rate is period-dated (seatPeriods,
    // lazily seeded). seatRate stays only as the pre-rewrite fallback.
    seatRate: 0,
    seatPeriods: [],
