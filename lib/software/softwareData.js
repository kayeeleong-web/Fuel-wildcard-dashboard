 ...
 *
 * Usage/perSeat are still a single flat rate each — a real, scoped-down decision, not an
 * oversight: those two already vary month to month on their own (usage count, headcount)
 * without needing a second axis of change. percentRevenue got the same period treatment
  ...
  return costItems.map((item) => {
    ...
    const percentPeriods =
      item.driverType === 'percentRevenue' && (!item.percentPeriods || !item.percentPeriods.length)
        ...
        : item.percentPeriods;
    const withPeriods = { ...item, periods, percentPeriods };
